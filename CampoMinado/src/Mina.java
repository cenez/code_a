...
public class Mina extends JButton{
	...
	public Mina(int l, int c) {
		L = l; C = c; isBomba = getRandom(2)==1?true:false;
	}
	...
	public void setPerimetro(Mina[][] minas) {
		...
		for(int i=0;i<len;i++) {
			for(int j=0;j<len;j++) { //Observar logica no arquivo: logica.png
				if(L-1==L-i && C-1==C-j) if((L-1)>=0 && (C-1)>=0)
					perimetro[0] = minas[L-i][C-j];
				if(L-1==L-i && C==j)     if((L-1)>=0)
					perimetro[1] = minas[L-i][j];
				if(L-1==L-i && C+1==C+j) if((L-1)>=0 && (C+1)<len)
					perimetro[2] = minas[L-i][C+j];
				if(L==i && C-1==C-j)     if((C-1)>=0)
					perimetro[3] = minas[i][C-j];
				if(L==i && C+1==C+j)     if((C+1)<len)
					perimetro[4] = minas[i][C+j];
				if(L+1==L+i && C-1==C-j) if((L+1)<len && (C-1)>=0)
					perimetro[5] = minas[L+i][C-j];
				if(L+1==L+i && C==j)     if((L+1)<len)
					perimetro[6] = minas[L+i][j];
				if(L+1==L+i && C+1==C+j) if((L+1)<len && (C+1)<len)
